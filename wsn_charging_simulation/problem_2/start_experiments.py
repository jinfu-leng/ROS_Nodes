...
param_res_file_name = 'res_prob2_outdoor_center_real_data2.csv'
...
for num in param_number_nodes:
	for size in param_ground_size:
		for setup_time in param_localization_time:
			for net_type in param_network_type:
				...
				networ_type_str = str(num) + '_' + str(size) + '_' + str(size) + '_' + str(net_type) + '_' + str(setup_time)
				for experiment_time in range(param_experiment_time):
					...
					for UAV_mode in param_UAV_modes:
						...
						nodes = copy.deepcopy(nodes_new)
						round_num = 0					
						print 'Network Type: ' + networ_type_str
