...
//-----------------------------------------------------------------------------
{
	...
	/// \brief This feature will ALWAYS execute internal update callbacks and will treat each write attempt to this feature
	/// as a value different from the current one.
...
//-----------------------------------------------------------------------------
{
	...
	/// \brief Issued when a property list is updated from a file or (under Windows&reg;) the Registr&reg; and
	/// some of the parameters cannot be applied to the properties within the list.
	///
	/// Possible reasons for this error being reported include:
	/// - a value stored in the file does not fall within the valid value range of the property
	/// - a data type of a certain feature does not match the data type in the property list
	/// - ...
	///
	/// \note
	/// This error will only be reported if the update operation was executed with the <b>mvIMPACT::acquire::sfFailOnInconsistentData</b>
	/// flag set.
	PROPHANDLING_NON_APPLICABLE_DATA_DETECTED = -2039,
	/// \brief Issued when a property list is updated from a file or (under Windows&reg;) the Registr&reg; and
	/// features are detected, which are just present either in the file or the property list to update.
	///
	/// \note
	/// This error will only be reported if the update operation was executed with the <b>mvIMPACT::acquire::sfFailOnInconsistentData</b>
	/// flag set.
	PROPHANDLING_UNPROCESSED_COMPONENTS_DETECTED = -2040,
...
//-----------------------------------------------------------------------------
{
	...
	/// \brief If set an error will be reported if a setting contains inconsistent data.
	///
	/// An error will be reported when updating an existing setting from a file or(under Windows&reg;) the
	/// Registry&reg; and
	/// - the exisiting setting contains features which are not present in the database it is updated from
	/// - the exisiting setting does \b NOT contain a feature that is present in the database it is updated from
	/// - at least one property in the exisiting setting does not support a value it is assigned from the database
	sfFailOnInconsistentData = 0x4000,
#ifndef DOXYGEN_SHOULD_SKIP_THIS
