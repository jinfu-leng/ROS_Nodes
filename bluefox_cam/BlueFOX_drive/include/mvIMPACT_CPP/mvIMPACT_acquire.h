...
#	endif // #ifdef MVIMPACT_H_
	...
	//-----------------------------------------------------------------------------
	/// \brief An exception thrown in case of a \b mvIMPACT::acquire::PROPHANDLING_NON_APPLICABLE_DATA_DETECTED error.
	class ENonApplicableDataDetected : public EPropertyHandling
	//-----------------------------------------------------------------------------
	{
	public:
		explicit ENonApplicableDataDetected( const std::string& path, const std::string& errorOrigin ) : EPropertyHandling("Non-applicable data has been detected during an update operation with file " + path, errorOrigin, PROPHANDLING_NON_APPLICABLE_DATA_DETECTED) {}
		virtual ~ENonApplicableDataDetected() throw() {}
	};

	//-----------------------------------------------------------------------------
	/// \brief An exception thrown in case of a \b mvIMPACT::acquire::PROPHANDLING_UNPROCESSED_COMPONENTS_DETECTED error.
	class EUnprocessedComponentsDetected : public EPropertyHandling
	//-----------------------------------------------------------------------------
	{
	public:
		explicit EUnprocessedComponentsDetected( const std::string& path, const std::string& errorOrigin ) : EPropertyHandling("Unprocessed components have been detected during an update operation with file " + path, errorOrigin, PROPHANDLING_UNPROCESSED_COMPONENTS_DETECTED) {}
		virtual ~EUnprocessedComponentsDetected() throw() {}
	};
	...
	{
	public:
		/// \brief Raises an exception from an error code, its origin, a string with additional information and an
...
#	endif
	...
	/// \b CaptureToUserMemory.cpp and have a look at the
...
PYTHON_ONLY(%immutable;)
	...
	/// This property will only contain meaningful data if the device supports \b HRTC and a program is running and writing data to
	...
	/// \brief A 64 bit integer property \b (read-only) containing the X-offset of the image in pixels as returned in the
	/// chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkOffsetX;
	/// \brief A 64 bit integer property \b (read-only) containing the Y-offset of the image in pixels as returned in the
	/// chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkOffsetY;
	/// \brief A 64 bit integer property \b (read-only) containing the width of the image in pixels as returned in the
	/// chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	PropertyI64 chunkWidth;
	/// \brief A 64 bit integer property \b (read-only) containing the height of the image in pixels as returned in the
	/// chunk data attached to the image.
	///
	/// \note
	/// For additional information about the chunk data format please refer to \ref ImageAcquisition_section_chunk.
	...
	/// An integer property \b (read-only) containing the width of the image in pixels.
...
public:
	...
	int imageRequestWaitFor(	/// [in] The maximum wait time in milli-seconds(ms) for
	...
	///
	/// \note This feature has been introduced in version 1.12.68 of this SDK. If this application has been compiled
...
#	endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
		...
		/// \brief Returns the number of \b mvIMPACT::acquire::PulseStartConfiguration objects available for the \b mvIMPACT::acquire::Device
...
#		endif // #ifdef DOTNET_ONLY_CODE
		...
		public:
			...
			///\endcode
			/// which will point to the same folder), under Linux&reg;
...
#			endif // #ifndef WRAP_PYTHON (In Python, object assignment amounts to just a reference count increment anyhow; you need to call the constructor or possibly some slice operation to make a true copy)
			...
			/// \brief Returns the total number camera descriptions currently available for the device
			/// that constructed this instance of the class.
...
/// @}
	...
	inline void ExceptionFactory::raiseException( const char* pFunctionName, int lineNumber, int errorCode, HOBJ objectHandle /* = INVALID_ID*/, const std::string& additionalInfo /*= ""*/ )
	{
		...
		case PROPHANDLING_NON_APPLICABLE_DATA_DETECTED:
			throw ENonApplicableDataDetected(additionalInfo, oss.str());
		case PROPHANDLING_UNPROCESSED_COMPONENTS_DETECTED:
			throw EUnprocessedComponentsDetected(additionalInfo, oss.str());
		// unsupported:
