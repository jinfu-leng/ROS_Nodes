...
private:
  ...
  // the number of left chances for non-detection, otherwise a flag will be published to show that there is no ball in the image
  int left_chance;
...
BallDetector::BallDetector()
  :it(nh)
  {
    ...
    circle_pub_ = nh.advertise<ballDetector::ballLocation>("ballLocation", 1);
    ...
    debugImg4_pub_ = it.advertise("debug/img4",1);
...
void BallDetector::imageCb(const sensor_msgs::ImageConstPtr& msg){
  ...
  static cv::Mat ballPathImg;
  ballPathImg = cv_const_ptr->image.clone();
  cv::rectangle(ballPathImg,
                  cv::Point(0,0),
                  cv::Point(ballPathImg.cols,ballPathImg.rows),
                  cv::Scalar(255,0,0,0),CV_FILLED);
  //cv::Scalar lowThresh_path = cv::Scalar(255,255,255,0);
  //cv::Scalar highThresh_path = cv::Scalar(0,0,0,0);
  //cv::inRange(hsvImg,lowThresh_path,highThresh_path,ballPathImg);
#ifdef BALLDETECTOR_DEBUG
  ...
  if(idxMax >= 0){
    ...
    circ.x = rect.x + rect.width/2 - threshImg.cols/2;
    circ.y = -1*(rect.y + rect.height/2 - threshImg.rows/2);
    circ.radius = radius;   
    circ.nSecond = image_time.toNSec(); //Add timestamp
    circle_pub_.publish(circ);
#ifdef BALLDETECTOR_DEBUG
    ROS_INFO("Width %d Height %d",rect.width,rect.height);
    ...
    //And the center point to ballPathImg
    cv::circle(ballPathImg,
                  cv::Point(rect.x + rect.width/2,rect.y + rect.height/2),
                  2,
                  cv::Scalar(0,255,0,0),4);

    // reset left_change to initial value
    left_chance = 3;
#endif /** BALLDETECTOR_DEBUG **/
  }
  else{// when there is no ball is detected in continuous three frames, then publish a message with radius = -1 as a flag
    left_chance--;	
    if(left_chance<=0){
       ballDetector::ballLocation circ;
       circ.radius = -1;
  ...
  cv_img.image = ballPathImg;
  cv_img.encoding = enc::BGR8;
  debugImg4_pub_.publish(cv_img.toImageMsg());
#endif /** BALLDETECTOR_DEBUG **/
